 ...
 */
async function processElectronVueTools(selectedTools: string[], targetDir: string): Promise<void> {
   for (const tool of selectedTools) {
      switch (tool) {
         case 'eslint':
 ...
 * 处理 node 模板的工具配置
 */
async function processNodeTools(selectedTools: string[], targetDir: string): Promise<void> {
   for (const tool of selectedTools) {
      switch (tool) {
         case 'eslint':
